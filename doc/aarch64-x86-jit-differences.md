...
| `jit_emit` | **x86-64 の機械語 emission** | `x86_64 && !no-jit` |

つまり **aarch64 = `jit && !jit_emit`**。`#[cfg(jit_emit)]` が x86 専用、
`#[cfg(all(jit, not(jit_emit)))]` が aarch64 専用の lowering です。

モジュール分割（[jitgen.rs](../monoruby/src/codegen/jitgen.rs)）:

| 役割 | x86-64 (`jit_emit`) | aarch64 (`not(jit_emit)`) |
|------|--------------------|---------------------------|
...
[`patch.rs` `compile_patch`(`jit_emit`)](../monoruby/src/codegen/patch.rs):
...
[`patch.rs` `compile_patch`(`not(jit_emit)`)](../monoruby/src/codegen/patch.rs) +
...
| recompile API | `recompile_method` 等（[compiler.rs](../monoruby/src/codegen/compiler.rs)、`jit_emit`） | 無し |
...
- **aarch64**: recompile が無いので、**整数の定数畳み込みの直前に `AsmInst::CheckBOP` を
  ...
  `cfg(not(jit_emit))`）し、[`compile_stub.rs`](../monoruby/src/codegen/jitgen/asmir/compile_stub.rs) が
...
- **インライン ASM**（`InlineFuncInfo::InlineGen`）は `jit_emit` 限定
