...
- `spec/skip.txt`: **5 ファイル**（救えない CRASH / プロセス死のみ）:
  `core/enumerator/new`、`core/exception/{signal_exception,signm,signo}`、
  `library/expect/expect`。
- `spec/tags/`: **6 ファイル / タグ付き example 7 件**:
  - `core/argf/read`（`/dev/zero` 無限読み）、`core/argf/readlines`、
    `core/io/copy_stream`、`core/io/select`（2件）、
    `library/socket/socket/{tcp,udp}_server_loop`。
  - CI 側で tags のコピー先を `spec/tags/ruby/` に修正した（上部「更新」参照）ので
    `mspec ci` がタグを読み、fast-path でハングする example だけを除外して完走する。
