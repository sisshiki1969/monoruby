...
(§6.2), which pair with `ensure_end_spliced` rather than with this call. The
...
`ensure`, and 14% on §6.2's find-first shape.

What remains on that path is larger than what this removed: a protected region
still loads `$!` at entry through the generic hooked-global runtime call
(`:00001 %2 = $(errinfo)`), once per invocation, although `$!` is a plain
`Executor` field.

### 6.2 Spliced non-local exits (issue #1185)
...
| JIT-spliced non-local exits (§6.2)   | `../monoruby/src/codegen/jitgen/context.rs` (`try_splice_exit`), `jitgen/compile.rs` (`emit_spliced_exit`), `jitgen/compile/method_call.rs` (`emit_spliced_landing`) |
