...
(b) の「直接の親」は常にバックトレースに出る — 失敗した `mark` を呼んだのは
親の `mark_children` フレームだからである。ただし §6.3 のマークキューに積まれた
地点より**上の祖先はバックトレースから切れる**ので、DEAD 検出点は
`scanning from: <ptr> ty=...`(`Allocator::mark_referrer`)としてその
キュー entry を併せて出力する。`scanning from: root set` なら、走査はまだルート集合の
中にいた — つまり壊れた辺はオブジェクトの中ではなくルート側(ビルトインの
未ルート一時値、古いフレームスロット)にある。
