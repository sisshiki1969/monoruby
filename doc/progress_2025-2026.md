...
- **one shared native-call primitive set** (Aug 2026) — `Fiddle.___dlopen` /
  `___call` / `___read` / `___write` / … (`src/builtins/fiddle.rs`) are now the
  single place C is reached from. `gem/ffi_c.rb` calls them instead of a
  duplicate registration under `FFI` (which also gains the JIT inliners for
  `___read` / `___write`), and the `sqlite3` bridge calls them directly rather
  than through `FFI::Library`, so it no longer needs the host `ffi` gem.
