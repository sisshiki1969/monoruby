...
| [`ruby_bench_fluentd_lobsters_2026-09.md`](ruby_bench_fluentd_lobsters_2026-09.md) | JA | design record | Why ruby-bench's fluentd and lobsters exit 1: C-extension `LoadError`s at the door (yajl / msgpack / cool.io / strptime / zstd; bcrypt / markly / `SQLite3::Backup`), and behind them twelve language and stdlib gaps found by stubbing the extensions and driving both benchmarks to completion — `(...)` forwarded from a lambda literal, `extend` on a singleton class, BINARY regexps built from `\xHH` escapes, anonymous `*`/`**` from a nested scope, `Time#to_date`, `unpack1("@3E")`, `Time#<=` through a redefined `<=>`, recycled `object_id`s under finalizers — all fixed, with what the extension stand-ins would take. |
...
| [`lobsters_profile_2026-09.md`](lobsters_profile_2026-09.md) | JA | design record | Where lobsters spends its steady-state time at 0.67x of YJIT: `perf` breakdown, uprobe call counts, the mimalloc A/B, JIT code bloat, and why half of the top deopts come from polymorphic sites whose classes all resolve to one ISeq (neither the class-set guard nor the PIC takes them), compared with how ZJIT splits such sites into per-type arms. |
...
| [`native_extension_loading.md`](native_extension_loading.md) | JA | plan | How to stop bundling every native library in the core binary (17.5 MB of C source, 6.6 MB of linked objects, 11k lines of Rust glue): a CRuby C-API layer, a monoruby-specific C ABI for dlopen'd C / Rust extensions, system shared libraries, feature gating, out-of-process, WASM and Fiddle-backed Ruby compared, with a staged recommendation — feature-gate, then move the existing sqlite3 / zlib / zstd / psych / nokogiri halves behind a `mr_*` C ABI into `~/.monoruby/v<ver>/ext/`, then a CRuby shim on top if needed. §7 covers how an install is actually produced: the extensions as Cargo artifact dependencies (so `cargo install` alone suffices), and the relocatable tree `bin/dist` stages. |
