...
残りは `ClassInfo`（4,058 個、定数 3,736・クラス変数 54）と frozen 文字列プール。
`ClassInfo` は定数とクラス変数が変異するので逆引きでは減らせず、ダーティビット +
store 側リメンバードセットが要る。オブジェクト化が本当に必要になるのは
「死んだ ISeq / 無名クラスの回収」の方で、これは性能ではなくメモリリーク
（`iseqs` / `functions` / `constsite_info` が append-only）の課題として別に扱う。
