...
| 5 | `respond_to?` がメソッド未検出時に呼ぶ `respond_to_missing?` を、ISeq の `ConstReturn` ヒント（既定の `Object#respond_to_missing?` は `false`、リテラルを返すユーザ定義も同様）から Ruby 呼び出しなしで答える。計算する本体は従来どおり `(name, include_all)` で呼ぶ | `builtins/kernel.rs::respond_to` | 多相受信側の未検出 80 → 41 ns（YJIT 84）、ユーザ定義混在 84 → 63 ns（YJIT 107） | 下表 |
