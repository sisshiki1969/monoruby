...
> **「Range は frozen だから書き換わらないのでは」への回答**: frozen になるのは
> *直接の* `Range` インスタンスだけで、しかも `initialize` の**最後**。
> `Range.allocate` は未初期化・**未 frozen** の Range を返すので、
> 「allocate → 3 回生き延びて昇格 → あとから initialize が young な値を格納」
> という窓が実在する（サブクラスは最後まで unfrozen）。既存のバリアが実際に
> それを支えていることを確認した — `Range#initialize` の
> `write_barrier(start)` / `(end)` を外したビルドと外さないビルドを
> gc-stress で 8 回ずつ走らせると、**外した方は 8/8 で `DEAD RVALUE` abort、
> 外さない方は 0/8**（再現スクリプトは、初期化に渡す String をローカル変数にも
> 呼び出し元のフレームスロットにも残さないよう、別メソッド内で格納してから
> そのフレームを pop させる形にする必要がある — さもないと young 側がフレーム
> ルートから到達可能で、バリアの有無に関わらず生き残ってしまう）。
...
まず「昇格できないから残っている」のか「本当に短命」なのかを実測した。全生存者を
加齢し（昇格は従来どおり `is_promotable` で制限）、マーク時点の age 分布を取る:

| | 総数/minor | age 0 | 1 | 2 | 3 | 4 | 5 | **6+** |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| erubi PROC | 2,697 | 18 | 18 | 18 | 18 | 0 | 18 | **2,604 (96.5 %)** |
| erubi FRAME | 2,696 | 18 | 18 | 18 | 18 | 0 | 18 | **2,603** |
| AR PROC | 2,949 | 41 | 39 | 41 | 42 | 5 | 39 | **2,737 (93 %)** |
| AR FRAME | 2,868 | 40 | 39 | 41 | 41 | 4 | 39 | **2,662** |

対照として昇格可能な型（STRING / OBJECT / HASH / ARRAY）は age 0〜2 に集中し
**age 3 以上がゼロ** — 3 で昇格して再走査対象から外れるから。計測が正しいことの
裏付けにもなっている。

**結論: PROC / FRAME は短命ではない。** 93〜96 % が 6 回以上の minor GC を生き延びて
おり、昇格できないせいで**永久に毎回再走査されている**。erubi の minor 走査の 82 %
はそのまま削減余地であって、「どうせ死ぬものを見ているだけ」ではない。

**では何が障害か。** `Lfp::move_frame_to_heap`（`executor/frame.rs:634`）を読むと:

```rust
let mut heap_lfp = wrap_promoted_frame(src.to_vec());
heap_lfp.meta_mut().set_on_heap();
cfp.set_lfp(heap_lfp);          // ← 実行中フレームの LFP をヒープ側に張り替える
self.meta_mut().set_invalidated();
```

昇格は**実行中のフレームに対して**起き（ブロックがキャプチャした時点）、以後
その메소드のローカル変数代入は r14 経由でヒープバッファを書く。CRuby は捕捉された
env への書き込みが `vm_env_write` という別経路に分かれているのでそこだけバリアすれば
済むが、**monoruby にはその分離が無い**。したがって素直にやると「ヒープフレームへの
書き込み」＝「ローカル変数代入全部」になり、インタプリタと JIT 両バックエンドの
最もホットなパスにヘッダビット検査が乗る。

設計の選択肢（未着手・要判断）:

1. **全ローカルストアにバリア** — 素直だが最もホットなパスにコストが乗る。
   GC の削減（minor マークの 82 %）と実行時コストのトレードオフで、CLASS のような
   一方的な得ではない。実測しないとどちらが勝つか分からない。
2. **ストア時に `meta.on_heap()` を検査**して分岐 — 検査自体がホットパスのコスト。
3. **「所有フレームが return 済み」のフレームだけ昇格** — 上の 2,600 個は所有者が
   とうに return したクロージャなので、これで大半が取れる。return 後に残る書き込みは
   ネストしたブロックからの dynvar ストアだけで、そちらは稀なので安くバリアできる。
   ただし「return 済み」を表す仕組みが現状無く、それを作るところからになる。

3 が筋が良さそうだが、いずれにせよ CLASS / Time の類とは規模が違う変更になる。

### 8.7 補足: 「昇格した親の children も連鎖的に old にできるか」

できるし、`is_promotable()` な子に限れば健全（そうでない子を old にすると、その子の
非バリア経路が追跡不能な old→young 辺を作る）。ただし測ると取り分がほぼ無い。

1. **現行の remember-on-promote が 3 サイクル遅れで同じ状態に収束する。** 親は
   remembered set に入り、`mark_remembered` が毎 minor で子をマークするので子は
   普通に加齢して age 3 で昇格し、`reclassify_remembered` が親を外す。連鎖化は
   約 3 回ぶん早めるだけ。
2. **remembered set の実コストが小さい**（`mark_remembered` の時間と件数）:

   | ベンチ | 件数/minor | `mark_remembered` | minor マーク比 |
   |---|---:|---:|---:|
   | erubi | 11 | 12.3 µs | 1.2 % |
   | rack | 17 | 19.8 µs | 2.0 % |
   | graphql | 13 | 18.4 µs | 1.9 % |
   | **activerecord** | **158** | **260.8 µs** | **11 %** |

3. **唯一大きい activerecord の 158 件は連鎖化では届かない。** 1 件あたり 1.65 µs で、
   効いているのは件数ではなく再走査される中身の大きさ。これらは「今まさに昇格する
   親」ではなく、**とうに old になったコンテナが新しい young 要素を格納し続けて
   再登録されている**もの（＝書き込みバリアが設計どおり働いている状態）。昇格時の
   連鎖化は過渡期の親にしか作用しない。
4. Range / Complex / Rational に限れば §8.6 のとおり残っていない（RANGE 29 → 0 等）。

コスト面では、連鎖化は age 3 の閾値を迂回する。この閾値は「たまたま 1 回生き延びた
だけの短命オブジェクトを昇格させない」ためにあるので、汎用機構にすると、たまたま
その瞬間コンテナに入っていた短命オブジェクトを old にして major まで回収されない
浮遊ゴミを作る。

実体のある残件は activerecord の 11 % の方で、そちらは「オブジェクト単位ではなく
スロット単位で覚える」（カードマーキング）方向の話になり、連鎖化とは別の設計。
