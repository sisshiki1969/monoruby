...
3. **deopt / recompile-deopt / error 側出口**は `get_write_back()` を使い、これは
   ...
   #### 試して戻した案: `S`/`Sf` 進入で `valid_home = true`

   `valid_home` は実体化の経路でしか立たないので、コンパイル済みコードが
   自分でストアしたスロットは `false` のまま残る。安全側ではあるが不正確で、
   **store → discard** の経路で問題になる: スロットは home に良い値を持った
   まま `V` に戻り、以降の write-back が毎回 nil で塗り直す。`S`/`Sf` は
   「値が home にある」という意味そのもの(deopt が `S` スロットを home から
   読み戻すのがこの不変条件)なので、`set_mode` で立ててよいはずだった。

   実装して計測した結果、**正しいが遅い**ので戻した。

   - 健全性は確認済み: `frame-poison` 単独(3312 tests)、
     `gc-stress,frame-poison`(3311 tests)とも green。home が POISON のまま
     だったならセーフポイント毎 GC が捕まえる。
   - 発行ストアは減る: `optcarrot --opt` 1 回の同一地点で 7,709 → 7,292
     (-5.4%、辺の bridge は -19.8%)。
   - それでも遅い。独立した 2 バッチ(各 26 回、最大値基準)で分布に重なりなし:

     | | 規則なし | 規則あり | |
     |---|---|---|---|
     | バッチ 1 | 891.5 fps | 866.6 fps | -2.8% |
     | バッチ 2 | 895.6 fps | 874.4 fps | -2.4% |

   フレーム数を 180 → 2000 に増やすと差は ~1.4% に縮み分布も重なるので、
   **コンパイル時間の寄与**が疑わしい — `set_mode` はコンパイル中もっとも
   呼ばれる関数のひとつで、そこにテストとストアが 1 つ増える。もう一つの
   候補(排除できていない): home を早く valid にすると実体化が減り、それに
   伴う **`V` → `C(nil)` 昇格も減る**。join では片側が `V` なら Discard、
   `C(nil)` 同士なら定数として合流する(`join.rs`)ので、dead スロットへの
   nil ストア 1 個は `C(nil)` というモードを買う対価でもあった、という筋。

   どちらも確定できておらず、上振れは全実行で約 400 ストアにすぎないので、
   計測に従って戻した。
