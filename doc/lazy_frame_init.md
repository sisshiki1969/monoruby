...
`optcarrot --opt` では **master 比 -1.6% 程度の退行が残っている**(最大値基準、
8 反復。ホストのノイズが大きく中央値では解像しないため最大値で比較した):

| ビルド | vs master(最大値基準) |
|---|---|
| lazy init(`V`→`C(nil)` 導入前) | -2.5% |
| `V`→`C(nil)` 導入後 | -1.6% |
| さらに deopt の `wb_void` を外す(**不健全、計測専用**) | -0.45% |

原因は Phase 1 で `get_write_back()`(deopt / error 側方出口)に加えた
`wb_void` である。fill の除去自体は `--opt` では得になっていない — fill を
戻した `frame-poison` ビルドが lazy と同帯域だった。`--opt` は
specialized/inline コンパイルが支配的で prologue fill を払う箇所が元々少なく、
一方こちらは**ガードごとの側方出口**が未実体化 `V` スロットへの nil ストアを
抱えるため、差し引きで負ける。

想定される対処(未着手): 側方出口ごとに per-slot ストアを並べる代わりに、
`next_sp .. reg_num` の**範囲埋め**(定数サイズの数命令)にするか、void リストを
deopt サイトの側テーブルに置いて共有 Rust ハンドラに埋めさせる。
