...
fd を持たない旧 `blocking_region` は PR6 で全サイトが `blocking_io_region` に統合され削除)。
グリーンスレッド統合はこの上に載る:
...
1. **タイムスライス・プリエンプションなし**: busy-loop するスレッドは
...
2. **シグナルは「ポーリングしたスレッド」で変換される**(CRuby は main に配送)。
3. `Thread.new` のサブクラスは Ruby の `initialize` オーバーライドを実行しない。
4. `Thread#priority` は保存のみ(スケジューリングに影響しない)。
   `ThreadGroup` / `native_thread_id` / `fork` との相互作用は未実装。
5. 真の並列化は別の話(Ractor 型の分離が現アーキテクチャ —
   OS スレッドごとの ALLOC / CODEGEN / SCHEDULER — と整合的)。

(解決済み: `Thread.handle_interrupt` マスキングは PR5 で、mid-operation の
IO ブロックは §7 の would-block エミュレーション(PR6)で実装された。)
