...
| Ir/req（定常状態、WARM=12,000、§7.2 の注意） | <<STEADY_MONO>> | ≈ 2.87 M（GC 除き 2,653,417 実測、GC ≈ 218 k 推定） | <<STEADY_RATIO>> |
...
monoruby は GC 回数が最初から一定（1 回/200 req）なので早期と定常の差は小さい
（GC 除き 2,810,759、GC 246,693）。**定常状態で比べると monoruby の命令数は CRuby の
≈ 1.07 倍**で、実時間の 1.4 倍との差の残りが §7.6 の命令キャッシュである。
