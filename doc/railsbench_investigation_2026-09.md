...
（`Hash#[]` のキークラス）、そして (4) 直線的に増え続ける RSS（≈ 1.2 KB/req）である。**
...
| Ir/req（定常状態、WARM=12,000） | <<STEADY_MONO>> | <<STEADY_RB>> | <<STEADY_RATIO>> |
...
1 になるぶん Ir も減る。定常状態（WARM=12,000）で取り直した値が §7.1 の 2 行目。
...
| | monoruby | CRuby+YJIT |
|---|---:|---:|
| I1 ミス（`I1mr`） | <<I1_MONO>> | 28.02 |
| D1 読みミス（`D1mr`） | <<D1_MONO>> | 8.66 |
| LL ミス（`DLmr`） | <<DL_MONO>> | 0.67 |
| 条件分岐予測ミス（`Bcm`） | <<BCM_MONO>> | 12.79 |
| 間接分岐予測ミス（`Bim`） | <<BIM_MONO>> | 5.13 |
