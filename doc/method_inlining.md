...
### 4.2b ストアスケジューリングと swap パターン(`Hold`)

本体は中断不能なので **中間の ivar 状態は観測不可能**。これを利用して:

- **デッドストア除去**: 各 ivar の最後のストアのみ発行する。
- **並べ替え**: 「`PendingIvar(X)`(= X の初期値読み)は X へのストアより
  前」という依存だけ守れば、ストアは自由に並べ替えられる。非循環の衝突
  (`@a = @b; @b = @c` 等)はこれだけで解決する。
- **循環の解体**: 各ストアのソースは1つなので read-initial 依存グラフは
  関数的であり、循環(swap / rotate)は互いに素。循環1つにつき、入口
  ivar の初期値を **未バインドの GP プールレジスタ1本**
  (`ScheduledOp::Hold`, `alloc_scratch_gp`)に保持すれば、残りは逆依存順に
  連鎖して1レジスタで足りる(`t=@x; @x=@y; @y=@z; @z=t` も Hold 1回)。
  戻り値が上書き済み ivar の初期値を返すパターン(`t=@a; @a=v; t`)も
  同じ機構で扱う。

保持レジスタは register file 上「空き」のままなので、Hold から最終消費まで
の窓でレジスタを壊すものがあってはならない。窓内に現れ得るのは inline
ストア・純粋ロード・`lit2reg` のみで、いずれもプールレジスタを触らない。
唯一の危険は heap 型ストアの cold path(`set_ivar` 呼び出し)なので、
**Hold を含むプランに heap 型ストアが混じる場合**と、**Hold が同時に2本
必要になる場合**(循環+別 ivar の初期値リターン等)はスケジューラが棄却し、
specialization にフォールバックする。
...
- ~~**`swap!` パターン**~~: 実装済み(§4.2b のストアスケジューリング +
  `Hold`)。スロット確保は不要になった。残る棄却ケースは「Hold + heap 型
  ストアの混在」と「Hold 2本の同時生存」のみで、必要ならフレーム拡張による
  スクラッチスロットで解ける。
