...
impl SlotState {
    ...
    pub(super) fn set_stack_from_literal(&mut self, slot: SlotId, v: Value) {
...
impl AsmIr {
    pub(super) fn into_stack(&mut self, bb: &mut BBContext, slot: SlotId) {
        match bb.slot(slot) {
            LinkMode::Xmm(xmm) => {
                ...
                bb.set_slot(slot, LinkMode::Stack, Guarded::Float);
