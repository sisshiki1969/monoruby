...
fn process_fork(
    ...
    // Hold every process-global lock across the fork, so the child
    // cannot inherit one another OS thread was holding, and snapshot
    // what the child has to reset (`crate::fork`).
