...
#[monoruby_builtin]
fn eq(_: &mut Executor, globals: &mut Globals, lfp: Lfp) -> Result<Value> {
    let lhs = lfp.self_val();
    Ok(Value::bool(eq_bool(globals, lhs.as_complex(), lfp.arg(0))))
}

#[monoruby_builtin]
fn ne(_: &mut Executor, globals: &mut Globals, lfp: Lfp) -> Result<Value> {
    let lhs = lfp.self_val();
    Ok(Value::bool(!eq_bool(globals, lhs.as_complex(), lfp.arg(0))))
}
...
#[monoruby_builtin]
fn complex_rect(_: &mut Executor, globals: &mut Globals, lfp: Lfp) -> Result<Value> {
