    ...
    if let Some(rhs) = state.is_fixnum_literal(args) {
        ...
        if rhs >= 0 {
            ...
            if k < 64 {
                ...
                let deopt = ir.new_deopt(state);
                ir.inline(move |r#gen, _, labels, _| { let d = r#gen.deopt_label(labels, deopt, DeoptCause::Static("shift overflow")); shl_overflow_zero(r#gen, &d) });
            }
        }
    } else {
        ...
        let deopt = ir.new_deopt(state);
        ir.inline(move |r#gen, _, labels, _| { let d = r#gen.deopt_label(labels, deopt, DeoptCause::Static("shift overflow")); r#gen.gen_shr(&d) });
    }
    state.def_reg2acc_fixnum(ir, GP::Rdi, dst);
    true
}
...
fn integer_shl(
    ...
    if let Some(rhs) = state.is_fixnum_literal(args) {
        ...
        if rhs >= 0 && rhs < 64 {
            ...
            let deopt = ir.new_deopt(state);
            ir.inline(move |r#gen, _, labels, _| { let d = r#gen.deopt_label(labels, deopt, DeoptCause::Static("shift overflow")); r#gen.gen_shl_rhs_imm(rhs as u8, &d) });
        } else if rhs < 0 {
            // n << -k: equivalent to n >> k
            let k = (-rhs) as u64;
        ...
        if let Some(lhs) = state.is_fixnum_literal(recv) {
            ir.inline(move |r#gen, _, labels, _| { let d = r#gen.deopt_label(labels, deopt, DeoptCause::Static("shift overflow")); r#gen.gen_shl_lhs_imm(lhs.get(), &d) });
        } else {
            ir.inline(move |r#gen, _, labels, _| { let d = r#gen.deopt_label(labels, deopt, DeoptCause::Static("shift overflow")); r#gen.gen_shl(&d) });
        }
    }
    state.def_reg2acc_fixnum(ir, GP::Rdi, dst);
    true
...
fn integer_rem_int_rhs(
    ...
    let deopt = ir.new_deopt(state);
    ir.inline(move |r#gen, _, labels, _| { let d = r#gen.deopt_label(labels, deopt, DeoptCause::Value(GP::Rdi)); r#gen.gen_int_rem(&d) });
