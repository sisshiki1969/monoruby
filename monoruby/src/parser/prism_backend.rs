...
impl<'pr> Lowerer<'pr> {
    ...
    fn lower_node(&mut self, node: &prism::Node<'pr>) -> Result<Node, MonorubyErr> {
        ...
        Ok(match node {
            ...
            prism::Node::ForwardingSuperNode { .. } => {
                ...
                // method's arguments. Modeled as `Super(None)`.
                // A literal block can still be attached
                // (`super { ... }`): that becomes an ArgList holding
                // only the block, with `zsuper` set so bytecodegen
                // keeps the implicit argument forwarding and only
                // overrides the block — without the marker it would
                // be indistinguishable from `super() { ... }`, which
                // passes *no* arguments.
    ...
    fn lower_call(&mut self, node: &prism::CallNode<'pr>, loc: Loc) -> Result<Node, MonorubyErr> {
            ...
            && method == "[]"
        {
            ...
            // A forwarded (`a.[](...)`) or `&`-delegating call must also
            // stay a real method call: the `Index` node carries neither
            // the runtime-arity forward nor a block slot.
            if kw_args.is_empty() && hash_splat.is_empty() && !forwarding && !delegate_block {
                return Ok(Node {
                    kind: NodeKind::Index {
                        base: Box::new(base),
                        index: args,
        ...
        // An explicit dot call that forwards arguments (`recv.+(...)`,
        // as generated by ActiveSupport's `delegate`) has an empty
        // static arg list but is NOT unary — its arity is only known at
        // runtime, so it must stay a generic method call.
        if block.is_none()
            && !delegate_block
            && let Some(recv) = receiver_opt.as_ref()
            && args.is_empty()
