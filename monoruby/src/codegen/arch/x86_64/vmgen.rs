...
impl Codegen {
    ...
    pub(in crate::codegen) fn gen_vm_handlers(&mut self) -> VmHandlers {
        ...
        //BcOp::Retry
        let retry = self.jit.get_current_address();
        let raise = self.entry_raise();
        monoasm! { &mut self.jit,
            movq rdi, rbx;
        ...
        //BcOp::Redo
        let redo = self.jit.get_current_address();
        let raise = self.entry_raise();
        monoasm! { &mut self.jit,
            movq rdi, rbx;
        ...
        VmHandlers {
            ...
            array: self.vm_array(),
            array_teq: self.vm_array_teq(runtime::array_teq as *const u8 as _),
            rescue_array_teq: self.vm_array_teq(runtime::rescue_array_teq as *const u8 as _),
    ...
    fn vm_defined_cvar(&mut self) -> CodePtr {
        let label = self.jit.get_current_address();
        self.fetch_addr_r15();
        monoasm! { &mut self.jit,
            movl rdx, [r13 - 8];
