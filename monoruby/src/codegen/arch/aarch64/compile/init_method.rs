...
impl Codegen {
    ...
    pub(in crate::codegen::jitgen) fn emit_init(
        ...
        // Lazy frame initialization (doc/lazy_frame_init.md): nothing is
        // written for the non-argument locals/temps — every observer
        // materializes them from the abstract state first. `frame-poison`
        // puts the fill back as POISON so an unmaterialized slot reaching
        // the GC names itself. Mirrors x86 `init_func`.
