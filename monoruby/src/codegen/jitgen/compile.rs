...
impl<'a> JitContext<'a> {
    ...
    /// (issue #1185): defer the unwind, tear the machine frames down to
    /// the suspended host's in-progress call site, and return into it
    /// tagged. The host's own compile emits the landing and the branch
    /// into its `ensure` body when that call site is reached again
    /// (`emit_spliced_landing`), so this path registers no return context
    /// of its own — the host's `EnsureEnd` delivers the value.
    ///
    fn emit_spliced_exit(
        ...
        // The degenerate fallback inside the instruction raises
